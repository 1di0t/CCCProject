            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if (IsUserIdExists(userId))
                {
                    try
                    {
                        ...
                        string query = $"UPDATE car_in_part_tb SET mission_oil = '{newMissionOilDate}', engine_oil = '{newEngineOilDate}', break_fluid = '{newBreakFluidDate}', coolant = '{newCoolantDate}' WHERE user_id = '{userId}'";
                        MySqlCommand cmd = new MySqlCommand(query, connection);
                        cmd.ExecuteNonQuery();
        ...
        private void button3_Click(object sender, EventArgs e) // 추가 버튼
        {
            ...
            if (IsUserIdExists(userId))
            {
                try
                {
                    ...
                    string insertQuery = $"INSERT INTO car_in_part_tb (user_id, mission_oil, engine_oil, break_fluid, coolant) " +
